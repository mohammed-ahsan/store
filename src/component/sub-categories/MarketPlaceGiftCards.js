...
export default function MarketPlaceGiftCards({
  categories,
          ...
          <div className="mb-10 flex">
            <div className="w-full lg:w-2/5">
              <h2 className="text-xl lg:text-2xl mb-2 font-serif text-left font-semibold flex items-center">
                ...
                {showingTranslateValue(categories[0]?.children[3]?.name)}
              </h2>
            </div>
          </div>
          ...
          <div>
            {loading ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-5">
                {categories[0]?.children[3]?.children
                  .slice(0, 6)
                  .map((category, index) => {
                    return (
                      <Link
            ...
            <div className="w-full h-auto mt-20 flex justify-center">
              {categories[0]?.children[3]?.children.length > 6 ? (
                <Link
                  href={`/showsuballcatagory/${categories[0]?.children[3]?._id}`}
